...
import { HeadlineBillboard, SPONSOR_ASPECT, SPONSOR_FRAME_PAD, sponsorShellClass } from "./SponsorShowcase";
  ...
  return (
    <div>
      <p className="font-display text-4xl tracking-tight text-yellow md:text-6xl lg:text-7xl">
        Dit krijgt u als hoofdsponsor
      </p>
      <p className="mt-4 max-w-2xl text-white/75">
        {tier.placement}. Wie €25.000 of meer stort, staat bovenaan — homepage, teller én live
        trekking. Niemand kan u missen.
      </p>
...
export function SilverHomeRow({ sponsors }: { sponsors: SponsorCardData[] }) {
  ...
  return (
    <div>
      <p className="font-display text-[0.8rem] tracking-[0.28em] text-yellow">
        Silver sponsor · {tier.priceLabel}
      </p>
...
export function BronzeHomeRow({ sponsors }: { sponsors: SponsorCardData[] }) {
  ...
  return (
    <div>
      <p className="font-display text-[0.8rem] tracking-[0.28em] text-yellow">
        Bronze sponsor · {tier.priceLabel}
      </p>
      <div className="mt-4 grid grid-cols-2 gap-2 md:grid-cols-4 md:gap-3">
        {slots.map((s, i) =>
          s ? (
...
export function SponsorPlacements({
  ...
  return (
    <div className="space-y-14">
      ...
      {g.gold.length ? (
        <div>
          <GroupLabel title="Gold sponsor · €10.000" />
      ...
      {g.silver.length ? (
        <div>
          <GroupLabel title="Silver sponsor · €2.500" />
      ...
      {g.bronze.length ? (
        <div>
          <GroupLabel muted title="Bronze sponsor · €500" />
  ...
  return (
    <div className="absolute bottom-4 right-4 z-10 max-w-[min(100%,22rem)] bg-yellow px-5 py-4 text-black md:bottom-6 md:right-6 md:px-6 md:py-5">
      <p className="font-display text-[0.65rem] tracking-[0.22em]">
        Live trekking i.s.m.
        {isExampleSponsor(sponsor.name) ? " · voorbeeld" : ""}
      </p>
